...
9. **SEED** — create `clonedSite`, publish it FIRST (publish-before-reference rule), then create + publish `clonedPage` shell (slug `home`, sections: []), then patch sections in via the **optimized seed protocol** (v1.2, targets ~8 min):
   a. Probe: try returning ONE small section directly from `javascript_tool`. If it comes back clean, use direct returns for all sections; only fall back to the escape channel on `[BLOCKED…]`.
   b. Escape channel (fallback): escape RAW field values (never the stringified JSON blob) to `\uXXXX`, write into a temporary `<pre>`, read via `get_page_text`, stash/restore the host page body ONCE for the whole batch.
   c. **Batch 2–3 sections per `patch_documents` call** (keep each call ≤ ~40KB), `_key` on every array member.
   d. NO mid-run length checks. One final integrity gate: `count(sections)` correct, template lengths == browser-measured, and per-section `"{{" occurrences == count(slots)`. Publish once at the end.
